...
st.caption("数据接口：Yahoo Finance (yfinance) | 原理：对数线性回归 + 1.5σ 通道 | 完全复刻你那张图")

# ================== 预设基金列表（已改成 Yahoo 支持的格式） ==================
etf_list = {
...
with st.sidebar:
    ...
    st.markdown("---")
    st.info("💡 数据自动更新，无需 API Key，每天收盘后刷新即可。")
...
def fetch_data(ticker):
    ...
    if df.empty:
        st.error(f"无法获取 {ticker} 数据，请稍后重试或检查网络。")
        st.stop()
    df = df.reset_index()
...
fig.add_trace(go.Scatter(x=df["datetime"], y=df["close"], 
                         name="基金净值/收盘价", line=dict(color="#1f77b4")))
...
st.markdown("---")
st.caption("原理：对数净值线性回归 + 1.5σ 通道。最新净值越靠近下轨越便宜，越靠近上轨越贵。")
st.caption("数据来源：Yahoo Finance（与中国基金净值高度一致）。")
